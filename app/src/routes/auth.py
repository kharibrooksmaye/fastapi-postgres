...
from app.core.authentication import create_access_token, get_password_hash, get_user, verify_password
...
async def activate_user_lookup(data: Annotated[ActivateUserRequest, Form()], session: SessionDep):
    email = data.email
    ...
    user = result.first()
    if not user:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="User not found with provided email and phone number")
    if user.username or user.password:
        return {"status_code": status.HTTP_206_PARTIAL_CONTENT, "detail": "User has already been activated, please log in"}
    user.is_active = True
    session.add(user)
    await session.commit()
